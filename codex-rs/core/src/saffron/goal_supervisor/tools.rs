    ...
    async fn handle_call(
        ...
        let value = match self.kind {
            ...
            Kind::Snooze => {
                ...
                if reason.is_empty() {
                    return Err(FunctionCallError::RespondToModel(
                        "reason must describe the external condition or scheduled boundary"
                            .to_string(),
                    ));
                }
...
impl Kind {
    ...
    const fn description(self) -> &'static str {
        match self {
            ...
            Self::Snooze => {
                "Snooze only when every unfinished part depends on a future deadline or external process that can change without parent action, and no useful parent action remains. Use supervisor_followup_parent for mixed work, unanswered questions, or missing user authority or information."
            }
            Self::Compact => {
                ...
                BTreeMap::from([
                    (
                        ...
                        JsonSchema::integer(Some(format!(
                            "Whole seconds to a known scheduled boundary or a bounded, evidence-based recheck of an external process, from 1 through {MAX_SNOOZE_SECONDS}."
                        ))),
                    ),
                    (
                        "reason".to_string(),
                        JsonSchema::string(Some(
                            "Concise observed external condition or scheduled boundary that prevents useful parent action."
                                .to_string(),
                        )),
                    ),
                ]),
