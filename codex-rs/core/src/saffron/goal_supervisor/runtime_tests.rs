...
async fn continuity_recovers_the_same_goals_previous_action() {
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "previous_action": {
                "followup": {
                    "delivered_message": "continue the accepted implementation",
                }
            },
            "previous_action_at_ms": 1_787_288_448_000_i64,
            "goal_timing": {
                "snooze_count_since_goal_created": 2,
                "snoozed_seconds_since_goal_created": 600,
            },
    ...
    assert_eq!(
        ...
        Some(PreviousAction {
            ...
            action: Action::Followup {
                delivered_message: "continue the accepted implementation".to_string(),
            },
...
async fn continuity_recovers_snooze_delay_and_reason() {
    ...
    assert_eq!(
        rendered["goal_timing"],
        serde_json::json!({
            "snooze_count_since_goal_created": 4,
            "snoozed_seconds_since_goal_created": 1_800,
        })
    );
}

#[tokio::test]
async fn committed_actions_accumulate_snooze_totals_for_the_active_goal() {
    let (_home, parent, store) = parent_with_saffron_store().await;
    let goal = test_goal(parent.thread_id);
    runtime(&parent).state.lock().await.goal_id = Some(goal.goal_id.clone());
    ...
    commit_action(
        ...
        Action::Snooze {
            delay_seconds: 60,
            reason: "build is still running".to_string(),
        },
    )
    .await;
    commit_action(
        ...
        &goal.goal_id,
        Action::Followup {
            delivered_message: "consume the completed worker".to_string(),
        },
    )
    .await;
    commit_action(
        ...
        Action::Snooze {
            delay_seconds: 120,
            reason: "build remains in progress".to_string(),
        },
    )
    .await;
    ...
    let continuity = store
        ...
        .await
        .expect("load persisted continuity")
        .expect("persisted continuity");
    assert_eq!(continuity.snooze_count, 2);
    assert_eq!(continuity.snoozed_seconds, 180);
}
...
async fn continuity_discards_an_action_from_a_replaced_goal() {
    ...
    store
        .set_goal_supervisor_continuity(&GoalSupervisorContinuity {
            ...
            action: GoalSupervisorAction::Followup {
                delivered_message: Some("continue".to_string()),
            },
            action_at_ms: 10,
            snooze_count: 1,
            snoozed_seconds: 60,
...
async fn continuity_filters_an_in_memory_action_from_a_replaced_goal() {
    ...
    state.previous_action = Some(PreviousAction {
        ...
        action: Action::Followup {
            delivered_message: "continue".to_string(),
        },
        action_at_ms: 10,
        snooze_count: 1,
        snoozed_seconds: 60,
...
#[test]
fn delivered_followup_context_has_a_hard_character_limit() {
    let message = "🦀".repeat(MAX_DELIVERED_FOLLOWUP_CHARS + 1);

    assert_eq!(
        bounded_followup_message(&message),
        "🦀".repeat(MAX_DELIVERED_FOLLOWUP_CHARS)
    );
}
...
#[test]
fn snooze_action_keeps_its_reason_in_continuity() {
    let action = Action::Snooze {
        delay_seconds: 900,
        reason: "Remote build is still running.".to_string(),
    };

    assert_eq!(
        serde_json::to_value(action).expect("serialize snooze action"),
        serde_json::json!({
            "snooze": {
                "delay_seconds": 900,
                "reason": "Remote build is still running."
            }
        })
    );
}
