...
const INSTRUCTIONS: &str = concat!(
    ...
    "active. Goal mode is autonomous. For optional clarification, make a supported, reversible ",
    "assumption and continue. If authority or information is indispensable, follow the ",
    "blocked-goal process. Yield for a snooze only when every unfinished part awaits a future ",
    "deadline or an external process that can change without parent action."
);
