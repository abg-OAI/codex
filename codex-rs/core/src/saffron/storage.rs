...
WHERE thread_id = ? AND goal_id = ?
    ...
    pub(super) async fn set_goal_supervisor_continuity(
        ...
        let (snooze_delay_seconds, snooze_reason, delivered_followup_message) =
            match &continuity.action {
                GoalSupervisorAction::Snooze {
                    delay_seconds,
                    reason,
                } => (
                    Some(i64::try_from(*delay_seconds)?),
                    Some(reason.as_str()),
                    None,
                ),
                GoalSupervisorAction::Followup { delivered_message } => {
                    (None, None, delivered_message.as_deref())
                }
                GoalSupervisorAction::Compact => (None, None, None),
            };
        sqlx::query(
            r#"
...
ON CONFLICT(thread_id) DO UPDATE SET
    ...
    delivered_followup_message = excluded.delivered_followup_message,
    snooze_count = excluded.snooze_count,
    snoozed_seconds = excluded.snoozed_seconds
            "#,
        )
        ...
        .bind(snooze_reason)
        .bind(delivered_followup_message)
        .bind(i64::try_from(continuity.snooze_count)?)
        .bind(i64::try_from(continuity.snoozed_seconds)?)
        .execute(&self.pool)
        .await?;
        Ok(())
    }
...
fn goal_supervisor_continuity_from_row(
    ...
    let delivered_message = row.try_get::<Option<String>, _>("delivered_followup_message")?;
    let action = match (action_kind.as_str(), delay_seconds, reason) {
        ("followup", None, None) => GoalSupervisorAction::Followup { delivered_message },
    ...
    Ok(GoalSupervisorContinuity {
        ...
        snooze_count: u64::try_from(row.try_get::<i64, _>("snooze_count")?)?,
        snoozed_seconds: u64::try_from(row.try_get::<i64, _>("snoozed_seconds")?)?,
    })
}
