...
fn execution_guards_count_active_v2_subagent_turns() {
    ...
    let first = control
        .execution_guard_for_thread(MultiAgentVersion::V2, &source, thread_id)
...
fn execution_guards_ignore_root_and_v1_turns() {
    ...
    assert!(
        control
            .execution_guard_for_thread(MultiAgentVersion::V2, &SessionSource::Cli, thread_id)
            .is_none()
    );
    assert!(
        control
            .execution_guard_for_thread(
