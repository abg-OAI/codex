...
async fn silent_root_preserves_tool_evidence_for_hidden_saffron_supervisor() -> Result<()> {
    ...
    let model_responses = responses::mount_sse_sequence(
        ...
        vec![
            ...
            responses::sse(vec![
                responses::ev_response_created("parent-check"),
                responses::ev_exec_command_call(
                    "scan-result-call",
                    "echo scan-complete-next-check-tomorrow",
                ),
                responses::ev_completed("parent-check"),
            ]),
            responses::sse(vec![
                responses::ev_response_created("parent-idle"),
                responses::ev_assistant_message("parent-message", ""),
            ...
            responses::sse(vec![
                ...
                responses::ev_function_call_with_namespace(
                    ...
                    &json!({
                        "delay_seconds": 3600,
                        "reason": "Deployment is still running; next status update is due in one hour."
                    })
                    .to_string(),
                ),
    ...
    tokio::time::timeout(Duration::from_secs(10), async {
        while model_responses.requests().len() < 5 {
    ...
    let parent_evidence = requests[2].function_call_output("scan-result-call");
    assert!(
        parent_evidence
            .to_string()
            .contains("scan-complete-next-check-tomorrow"),
        "the parent must have successfully received the external check result"
    );
    assert_eq!(
        requests[3].inputs_of_type("function_call_output"),
        requests[2].inputs_of_type("function_call_output"),
        "the supervisor must inherit evidence even when the parent finishes silently"
    );
    assert_eq!(
        requests[3].inputs_of_type("function_call"),
        requests[2].inputs_of_type("function_call"),
        "inherited tool results must retain their matching calls"
    );
    assert!(
        requests[0]
            .tool_by_name("saffron", "supervisor_snooze")
            .is_none(),
        "supervisor actions must remain unavailable to the parent"
    );
    assert_eq!(
        requests[3]
            .message_input_texts("developer")
            .iter()
            .map(|text| text.matches("# Saffron Goal Supervisor").count())
            .sum::<usize>(),
        1,
        "the helper must receive its supervisor instructions exactly once"
    );
    assert!(
        requests[3]
            .tool_by_name("saffron", "supervisor_snooze")
